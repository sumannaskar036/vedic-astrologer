...
def calculate_vedic_chart(name, gender, dt, tm, lat, lon, city, ayanamsa_mode):
...
with st.sidebar:
    ...
    d_in = st.date_input("Date of Birth", datetime.date(1993,4,23))
...
if st.session_state.current_data:
    ...
    st.markdown(
        f'<div class="header-box">Janma Kundali: {d["Name"]} 🙏</div>',
        unsafe_allow_html=True
    )

    st.code(d["Full_Chart"])
...
IMPORTANT: Write response in {lang_opt}.
...
"""
        ...
        if "candidates" in data:
            ...
            st.warning("The stars are silent. Check billing or quota.")
